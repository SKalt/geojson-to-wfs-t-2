 ...
 */
const geomToGml = makeTranslator(
...
function tag(ns, tagName, attrsObj, inner) {
  ...
  if (tagName) {
    ...
    throw new Error('no tag supplied ' + JSON.stringify({ ns, tagName, attrsObj, inner }, null, 2));
  }
}
...
function translateFeatures(features, params = {}) {
  ...
  for (let feature of features) {
    ...
    let fields = '';
    if (geometry_name) {
      fields += tag(ns, geometry_name, {}, geomToGml(feature.geometry, '', { srsName, srsDimension }));
    }
 ...
 * @return {Feature[]}
 */
const array = (...maybe) => (maybe[0].features || [].concat(...maybe)).filter(f => f);
/**
 ...
 */
const id$1 = (lyr, id) => /\./.exec(id || '') ? id : `${lyr}.${id}`;
/**
...
const typeName = (ns, layer, typeName) => {
  if (!typeName && !(ns && layer)) {
    throw new Error(`no typename possible: ${JSON.stringify({ typeName, ns, layer }, null, 2)}`);
  }
...
function Insert(features, params = {}) {
  ...
  return tag('wfs', 'Insert', { inputFormat, srsName, handle }, toInsert);
}
...
function Update(features, params = {}) {
  ...
  if (params.properties) {
    let { /* handle, */inputFormat, filter: filter$$1, typeName: typeName$$1, whitelist } = params;
    ...
    if (geometry_name) {
      fields += makeKvp(geometry_name, tag(ns, geometry_name, {}, geomToGml(params.geometry, '', { srsName })));
    }
    return wfs$1('Update', { inputFormat, srsName, typeName: typeName$$1 }, fields + filter$$1);
  } else {
...
function Delete(features, params = {}) {
  features = array(features);
  let { filter: filter$$1, typeName: typeName$$1 } = params; // TODO: recur & encapsulate by typeName
...
function Replace(features, params = {}) {
  features = array(features);
  let { filter: filter$$1, inputFormat, srsName } = unpack(features[0] || {}, params || {}, 'filter', 'inputFormat', 'srsName');
  let replacements = translateFeatures([features[0]].filter(f => f), params || { srsName });
  filter$$1 = filter(filter$$1, features, params);
  return wfs$1('Replace', { inputFormat, srsName }, replacements + filter$$1);
}
